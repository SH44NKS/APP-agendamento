...
export const TIPO_ICONE: Record<TipoServico, string> = {
  instalacao: "🔧",
  retirada: "↩",
  manutencao: "🛠",
};
export const STATUS_LABEL: Record<string, string> = {
  ...
  agendado: "Agendado",
  reagendar: "Reagendar",
  concluido_tecnico: "Concluído pelo técnico",
...
export function mensagemWhatsapp(os: OrdemServico, nomeTecnico: string) {
  const primeiroNome =
    os.cliente_nome.trim().split(/\s+/)[0] || os.cliente_nome;
...
export function linkGoogleAgenda(os: OrdemServico) {
  ...
  const p = new URLSearchParams({
    ...
    dates: `${formato(inicio)}/${formato(fim)}`,
    details: `Veículo: ${os.veiculo_modelo} (${os.veiculo_identificador})\nConsultor: ${os.consultor_nome}`,
    location: os.local,
  });
